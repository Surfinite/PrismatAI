...
**Constants** — BV ${BV}/HP (block value per soak) · **ATK = BV/R_HALF ≈ ${f2(ATK)}/pt** (derived: my block lands this defense phase, an attack a half-turn later) · THREAT ${THREAT}/atk.

**Clock** — full-turn interest R = 4/3 ≈ ${f2(R)} · half-turn R_HALF = √(4/3) ≈ ${f2(R_HALF)} with discount d² = ${f2(1 / (R_HALF * R_HALF))} · perpetuity P = 1/(1−1/R) = ${f2(geomPerp(1))} (once-per-turn stream).

**Resources (WILL)** — gold 1 · green ${f2(RES.green)} (Conduit) · blue ${f2(RES.blue)} (Blastforge) · red ${RES.red} (Animus) · energy ${f2(RES.energy)} (hedged, board-dependent). Green/blue/red are producer-derived = gold-cost / (output × 3).
...
**Mechanics (tunable)** — self-sac max(block,burst)+opt (atk-burst ${OPT_SELFSAC_ATK} / token-burst ${OPT_SELFSAC_TOKEN}) · token-spawn = production stream of created tokens (own +ours(token), build-time-discounted; opponent −block) · chill +targetAmount·ATK·${CHILL_COEFF} (flat, board-aware later) · drone-kill +denied-drone production (Deadeye charge) · undef −${UNDEF_PER_HP}/HP block · fragile −${FRAGILE_PEN} block · Xaetron's HP-funded click ignored (→ pure block).
