...
> **Optional parallel de-risk — state-identity validation (low-cost first; parallel, NOT a blocker).** Two complementary checks; F6 automation was assessed and **rejected** (`docs/scratch/prismata-ladder-f6-export-viability-report.md`).
> - **(1) C++↔JS parity on SELF-PLAY states (headless, scalable — the Step-6 gate).** Validates the JS extractor against the **C++ engine's** GameState. Strong, but it only covers **self-play-reachable** states — the dave C++ engine has **no human-replay importer** (only `ReplaySerializer`, which *exports* self-play games; it cannot replay human server `commandList`s — that's why the JS engine exists). So this gate does **not** touch human-corpus states.
> - **(2) Manual F6 spot-check on HUMAN states (the ONLY human-corpus validator).** Because (1) can't reach human-replay states, a small **fresh, random/stratified** sample of human replay codes, F6-dumped and run through `js_engine/oracle_diff.js`, is the only state-identity check on the actual `human_1800_v2` rehearsal corpus. Do ~10–20 dumps; clean → strong confidence; a diff → caught before the AWS spend. **Gotchas (from the viability report):** sample *fresh random* codes, NOT the 4 known-bad dumps in `docs/scratch/` (those drove the faithfulness fixes; VXGaI's 2-unit diff is a snapshot-alignment artifact, not a live bug); pass an explicit `actionIndex` to `oracle_diff` for a stable phase-boundary comparison; add the `timeRemainingMS` ignore-rule (JS −1 vs F6 0). To make the per-turn loop painless within a manually-loaded replay, resurrect `tools/capture_replay_states.py` from git `e98787d` (a Win32 F6/Shift-Right macro).
> - **Ceiling (why C++ is the deeper gate):** F6 dumps the AS3 client's *local recomputation*, so it only proves **JS ≡ AS3-engine computation** — it cannot catch a bug shared identically by both. The independent **C++ engine** is the real triangulator (catches shared JS/AS3 bugs) — but only on self-play states. Neither alone is complete; together they bound the risk.
> - **Do NOT build** the automated/unattended F6 exporter: live-spectator state-harvest is impossible (the AS3 wire carries *moves only*, no state blob), and automated F6 over replays is a brittle single-instance GUI macro on EOL Adobe AIR (GPU display, admin hosts-redirect, live-server dependency, ~hundreds/hr serial) — disproportionate to a ~£400 go/no-go. Only revisit if an external requirement (e.g. a paper) needs exhaustive client-truth.
...
## External-review addenda — measurement/eval (2026-06-03, Claude-desktop; MUST fold into Tasks 5–14)

A spec-lineage review flagged measurement issues that look fine until analysed and can manufacture a false result. Each is tagged with the task it modifies. **#1, #6, #7 are load-bearing — do them.**

- **A1 — Decouple EVAL search budget from self-play N; fix the regression gate (→ Tasks 4, 7, 14, §12). [important]** `RL_Eval`/`RL_Eval_iter0` currently inherit the self-play `N` (≈512, a throughput/label knob). Eval gates a *deployment-scale* spend, so candidate AND all anchors must run at one fixed **deployment-representative budget** (e.g. `MaxTraversals:100000`/7s), not `N`. Set `RL_Eval`/`RL_Eval_iter0` `MaxTraversals` to the deployment budget (decoupled from `RL_SelfPlay`). **And** in §12 compute `d_reg` from **`RL_Eval_iter0_general`** (net_k vs pre-RL net, *same config + same budget*), NOT from the narrow `DSNN_Mixed35_5var` baseline (different config@100k) — else a pure budget gap trips `d_reg < −Y` and blocks a GO (the exact false-negative the design fights). Narrow + STEAMAI stay trajectory yardsticks (don't gate on them).
- **A2 — Exploration is gated to the opening, but IG recurs all game (→ Tasks 3, 14). [important]** The sampler (τ *and* ε) only fires for `getTurnNumber() < K`; after K it's pure argmax, no ε. With OB on, turns 1–2 are book-forced → effective IG-exploration window ≈ turns 3–6, yet IG fire/skip is a per-turn decision. Add an optional **persistent small late-ε** (e.g. `EpsilonLate≈0.05` applied when `turnNumber ≥ K`, while τ→0 keeps the bulk greedy) so the target decision is actually explored mid/late-game — a losing exploratory IG-skip *is* the (state→outcome) signal RL needs; a little late label noise is acceptable here. (K units confirmed = player-turns ≈ spec's "plies".) **If axis-1 comes back flat, enable this FIRST** before the §14 escalation path. Requires a small Task-3 sampler tweak (gate ε separately from τ).
- **A3 — Group-sequential boundary for the PROMOTION gate (→ Task 7). [important]** `sequential()` peeks at 128→256→512 vs 0.5 → family-wise Type-I ≈10–12%. Tolerable for the final GO (asymmetry), but a false-positive *promotion* poisons the buffer. For the gating (candidate-vs-current-net) comparison use a Pocock-style constant-z boundary (nominal α′≈0.022/look) or give only the final look full α; interim looks a stricter z.
- **A4 — Analyse the paired design as paired (→ Task 7). [medium]** `wilson_ci` treats games as iid, ignoring the negative within-pair correlation from colour-swap → too-wide CIs → wasted games / spurious "inconclusive". Cluster inference at the **card-set level** (per-set scores + CI across sets, or block-bootstrap at the set level).
- **A5 — Root-entropy metric + calibrate ε (→ Task 13). [important]** `read_mean_root_entropy` uses raw `aivisits` (UCB1 counts) — that's a *search-health* check (label it as such), not self-play diversity. For diversity, compute the **effective** post-ε distribution `(1−ε)·visits^(1/τ) + ε·uniform`. And since ε is the *sole* load-bearing exploration lever (forced-≥1-visit is redundant), **sweep ε alongside N** in Task 13, judged on effective entropy + IG fire/skip spread — don't freeze ε=0.25 at a guess.
- **A6 — Perspective round-trip test (→ Tasks 5/6/14). [important]** `test_labels.py` (Python scale) + export-parity (PyTorch↔C++ forward value) do NOT catch a perspective inversion at the *train-label-convention ↔ search-interpretation* boundary (a net trained on inverted-perspective labels passes both yet plays anti-optimally — the historical P0/P1 bug class). Add: (a) the Task-5 parity check on ≥1 shared state with a **known asymmetric outcome** (P0 won, active=P1 that turn) so an inversion shows as a value mismatch (the structural "same-keys" fallback would NOT catch this); (b) an end-to-end assertion via `query_move.js` that a clearly-winning continuation scores higher through the loop's own path. Verify `outcome_p0` (C++ exporter, P0-perspective) matches the JS human corpus convention AND how `Eval.cpp`/`NeuralNet` interpret the net output for `maxPlayer`.
- **A7 — STEAMAI tally must count candidate IDENTITY, not the White seat (→ Task 7). [important]** Under `--player-switch` the candidate alternates seats; `parse_matchup_tally` reading `White:` as candidate wins would report ~half its true wins → every candidate looks uniformly terrible vs STEAMAI. Parse `matchup_clean.js`'s **seat-independent per-identity** win-rate (the `[Pair] Win Rates (seat-independent)` / `playerWinRates.playerA` block), not the `White:`/`Black:` seat tally. Tell: implausibly-low, iteration-flat STEAMAI rates ⇒ this bug.
- **A8 — STEAMAI at fixed modest N, not sequential (→ Tasks 7, 14). [medium]** STEAMAI is a trajectory yardstick that gates nothing and is the slowest path (JS-bridge per-turn); under sequential-to-512 on a ~50% matchup it always escalates + stays inconclusive, burning the most expensive eval. Use a fixed modest N + CI; reserve sequential testing for the gating comparison.
- **A9 — Pre-register a STOP condition (→ §14 / `rl_campaign.md`). [strategic]** The interpretation guard + kill-criteria always route to *escalate*, never *abandon*. Pre-register the evidence that would justify stopping (e.g. if the O6 candidate-policy-head escalation *also* comes back flat, that's the action space/approach being the limit, not measurement). Note the local false-positive cost is more than £400 (it buys the whole AWS campaign's engineering + monitoring + opportunity cost).
- **A10 — Reframe the rehearsal-decay rationale (→ Task 6, `rl_data.py` comment). [nit]** The 30%→10% decay is justified by "self-play accumulates," but the fixed W=5 sliding window keeps self-play volume roughly constant after iter W. Reframe it as a *confidence* schedule (trust self-play more as it strengthens), not an accumulation argument.
- **A11 — Put attack/resource fields in the shared-state parity assert (→ Task 5, folds into A6). [medium]** Attack-stripping + the resource→`p0_resources`/`p0_attack` reshape were two of the three skews that bit the JS extractor; assert those specific field *values* on the shared state, not just structural presence.

- **A12 — Steam-protocol eval paths use the SWF's narrow iterator, not our widened one (→ Tasks 7, 10, 13; SHARED prereq). [important]** Verified (`AITools.cpp:124–187`): the standalone exe resolves its move iterator BY NAME (`"HardIterator_Root"`/`"HardIterator"`) from the per-request `aiParameters` — and the standalone does NOT load `config.txt` at startup. So any RL component that drives the candidate through `matchup_clean.js`/`query_move.js` (anchor 3 STEAMAI; the O7 tactical suite; the calibration root-entropy) runs the **narrow** action space, NOT our widened `HardIterator_5var_IGopt_Root`. **Task 10 is hard-dependent** — IG-skip is only a candidate in the widened iterator, so without this the tactical suite can never see a "skip IG" move (always fires) and is broken. The config-loading tournament path (anchors 1–2, self-play) is fine. **Fix (shared with the live drop-in, see `docs/multicore-rl-coordination.md` §2a):** make the standalone load our `config.txt`/DSNN iterator config at startup so the widened iterator is resolvable by name for the Steam-protocol paths (verify `parseJSONValue` merges vs replaces the registry); alternatively inject the full widened iterator definition into the aiParameters blob from JS (DRY-worse). Build this before the Steam-protocol eval anchors.

**Confirmed strong (do not erode):** full-C++ self-play export for train/inference feature parity; the wide-untrained iter-0 anchor + offline batch-RL iter-0; contamination asserts; resolved-config-hash pinning; failing-test-first cadence; the "freeze all but the four allowed changes during the first campaign" discipline.
