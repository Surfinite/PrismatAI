"""flopflop comprehensive stats analysis.

Merges expert_replays.json + flopflop_replays.json (from FlopFlopCodes.txt),
deduplicates by replay code, then produces comprehensive stats.
"""
import json, os
...
# --- Load expert_replays.json (canonical format) ---
with open('c:/libraries/prismata-replay-parser/expert_replays.json', 'r') as f:
    expert_replays = json.load(f)

# --- Load flopflop_replays.json (fetched from S3 via FlopFlopCodes.txt) ---
ff_replay_path = 'c:/libraries/prismata-replay-parser/flopflop_replays.json'
ff_replays = []
if os.path.exists(ff_replay_path):
    with open(ff_replay_path, 'r') as f:
        ff_replays = json.load(f)
    print(f"Loaded {len(ff_replays)} replays from flopflop_replays.json")

# --- Find all flopflop account names ---
FF_NAMES = set()
for r in expert_replays:
    for pn in ['P1Name', 'P2Name']:
        name = r.get(pn, '')
        if 'flopflop' in name.lower():
            FF_NAMES.add(name)
# Also check flopflop_replays for name variants
for r in ff_replays:
    for pn in ['p1', 'p2']:
        name = r.get(pn, '')
        if 'flopflop' in name.lower():
...
print(f"Account names found: {FF_NAMES}")

# --- Build unified game list, deduped by code ---
seen_codes = set()
games = []

def _parse_time(val):
    """Convert startTime which may be epoch int, epoch string, or ISO string."""
...
def add_game(code, p1_name, p2_name, p1_rating, p2_rating, deck, result, start_time, duration, end_condition, time_condition, source):
    if code in seen_codes:
        return
    seen_codes.add(code)
    is_p1 = p1_name in FF_NAMES
    is_p2 = p2_name in FF_NAMES
    if not (is_p1 or is_p2):
        return
    ff_name = p1_name if is_p1 else p2_name
    ff_rating = p1_rating if is_p1 else p2_rating
    opp_rating = p2_rating if is_p1 else p1_rating
...
# Process expert_replays first (richer metadata)
for r in expert_replays:
    start_ts = r.get('StartTime', 0)
    end_ts = r.get('EndTime', 0)
    add_game(
        code=r.get('Code', ''),
        p1_name=r.get('P1Name', ''),
        p2_name=r.get('P2Name', ''),
        p1_rating=r.get('P1RatingIni', 0),
        p2_rating=r.get('P2RatingIni', 0),
        deck=r.get('Deck', []),
...
# Process flopflop_replays (from S3 fetch)
for r in ff_replays:
    add_game(
        code=r.get('code', ''),
        p1_name=r.get('p1', ''),
        p2_name=r.get('p2', ''),
        p1_rating=r.get('p1_rating', 0),
        p2_rating=r.get('p2_rating', 0),
        deck=r.get('deck', []),
        result=r.get('result', -1),
        start_time=_parse_time(r.get('startTime', 0)),
        duration=0,  # not available from S3 fetch
        end_condition=-1,  # not available from S3 fetch
        time_condition=0,  # not available from S3 fetch
        source='flopflop_codes',
    )

expert_count = sum(1 for g in games if g['source'] == 'expert')
codes_count = sum(1 for g in games if g['source'] == 'flopflop_codes')
print(f"After dedup: {len(games)} games ({expert_count} from expert, {codes_count} from FlopFlopCodes)")

print(f'\n=== FLOPFLOP COMPREHENSIVE STATS ===')
...
# Rating
ratings = [g['ff_rating'] for g in games if g['ff_rating'] > 0]
if ratings:
    print(f'\n--- RATING ---')
    ...
    recent_ratings = [g['ff_rating'] for g in sorted(games, key=lambda x: x['start_time']) if g['ff_rating'] > 0][-50:]
    print(f'Recent Rating (last 50 games avg): {statistics.mean(recent_ratings):.0f}')

# Win/Loss
decided = [g for g in games if g['ff_won'] is not None]
wins = sum(1 for g in decided if g['ff_won'])
...
decided_human = [g for g in human_games if g['ff_won'] is not None]
human_wins = sum(1 for g in decided_human if g['ff_won'])
print(f'Human-only games: {len(human_games)}, WR: {human_wins}/{len(decided_human)} = {human_wins/len(decided_human)*100:.1f}%')

# Win rate by position
p1_games = [g for g in decided if g['ff_position'] == 1]
p2_games = [g for g in decided if g['ff_position'] == 2]
p1_wins = sum(1 for g in p1_games if g['ff_won'])
p2_wins = sum(1 for g in p2_games if g['ff_won'])
if p1_games:
...
for lo, hi, label in tiers:
    ...
    if tier_games:
        tw = sum(1 for g in tier_games if g['ff_won'])
...
for opp, count in opp_counts.most_common(15):
    ...
    opp_wins = sum(1 for g in opp_decided if g['ff_won'])
...
print(f'\n--- BEST WIN RATE UNITS (min 15 games) ---')
...
print(f'\n--- WORST WIN RATE UNITS (min 15 games) ---')
...
ff_resigned = sum(1 for g in resign_games if not g['ff_won'])
opp_resigned = sum(1 for g in resign_games if g['ff_won'])
print(f'  FF Resigned: {ff_resigned}, Opponent Resigned: {opp_resigned}')

elim_games = [g for g in decided if g['end_condition'] == 1]
ff_eliminated = sum(1 for g in elim_games if not g['ff_won'])
opp_eliminated = sum(1 for g in elim_games if g['ff_won'])
print(f'  FF Eliminated opp: {opp_eliminated}, Got Eliminated: {ff_eliminated}')

# Rating trajectory
print(f'\n--- RATING TRAJECTORY ---')
sorted_games = sorted(games, key=lambda x: x['start_time'])
first_100_r = [g['ff_rating'] for g in sorted_games[:100] if g['ff_rating'] > 0]
last_100_r = [g['ff_rating'] for g in sorted_games[-100:] if g['ff_rating'] > 0]
...
for g in sorted_games:
    if g['ff_rating'] > 0 and g['start_time'] > 0:
        d = datetime.fromtimestamp(g['start_time'])
        q = f'{d.year}-Q{(d.month-1)//3+1}'
        quarterly[q].append(g['ff_rating'])
...
# Streaks
print(f'\n--- STREAKS ---')
sorted_decided = [g for g in sorted_games if g['ff_won'] is not None]
best_win_streak = 0
best_loss_streak = 0
cur_win = 0
cur_loss = 0
...
# Biggest upsets
print(f'\n--- BIGGEST UPSET WINS ---')
upset_wins = sorted([g for g in decided if g['ff_won'] and g['opp_rating'] > g['ff_rating']],
                     key=lambda x: x['opp_rating'] - x['ff_rating'], reverse=True)[:5]
for g in upset_wins:
    diff = g['opp_rating'] - g['ff_rating']
    print(f'  vs {g["opp_name"]} ({g["opp_rating"]:.0f}) when rated {g["ff_rating"]:.0f} (+{diff:.0f}) [{g["code"]}]')

print(f'\n--- BIGGEST UPSET LOSSES ---')
upset_losses = sorted([g for g in decided if not g['ff_won'] and g['opp_rating'] < g['ff_rating']],
                       key=lambda x: x['ff_rating'] - x['opp_rating'], reverse=True)[:5]
for g in upset_losses:
    diff = g['ff_rating'] - g['opp_rating']
    print(f'  vs {g["opp_name"]} ({g["opp_rating"]:.0f}) when rated {g["ff_rating"]:.0f} (-{diff:.0f}) [{g["code"]}]')

# Notable scalps
print(f'\n--- NOTABLE SCALPS ---')
big_wins = sorted([g for g in decided if g['ff_won']], key=lambda x: x['opp_rating'], reverse=True)[:5]
for g in big_wins:
...
for lo, hi, label in dur_tiers:
    ...
    if tier:
        tw = sum(1 for g in tier if g['ff_won'])
...
notable = ['Homeless', 'jamberine', 'Kolento', 'coffeeyay', 'Steel', 'Polari', 'Lycomedes',
           ...
           'Wonderboat', 'Surfinite']
print(f'\n--- HEAD-TO-HEAD vs NOTABLE PLAYERS ---')
for name in notable:
    pg = [g for g in decided_human if g['opp_name'] == name]
    if pg:
        pw = sum(1 for g in pg if g['ff_won'])
...
# Playing up vs down
print(f'\n--- PLAYING UP vs DOWN ---')
up_games = [g for g in decided_human if g['opp_rating'] > g['ff_rating'] + 50]
down_games = [g for g in decided_human if g['opp_rating'] < g['ff_rating'] - 50]
even_games = [g for g in decided_human if abs(g['opp_rating'] - g['ff_rating']) <= 50]
if up_games:
    up_w = sum(1 for g in up_games if g['ff_won'])
    print(f'  Playing UP (opp 50+ higher): {up_w}/{len(up_games)} = {up_w/len(up_games)*100:.1f}%')
if even_games:
    even_w = sum(1 for g in even_games if g['ff_won'])
    print(f'  Even match (within 50): {even_w}/{len(even_games)} = {even_w/len(even_games)*100:.1f}%')
if down_games:
    down_w = sum(1 for g in down_games if g['ff_won'])
