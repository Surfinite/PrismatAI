...
def main():
    ...
    parser.add_argument("--window-override", action="store_true",
                        help="Forensic: skip the C5 window-membership checks (INVALID "
                             "markers + rl_parent_* lineage stamps) on the --rl-mode "
                             "replay window.")
    parser.add_argument("--no-swa", action="store_true",
                        help="Disable SWA entirely (J1 re-freeze 2026-06-13: in a "
                             "~6-epoch RL fine-tune SWA averages 4 near-collinear "
                             "snapshots, measurably shrinking the update ~20%% for "
                             "no accuracy gain — training-02). A final_model.pt "
                             "(last-epoch weights, swa_model.pt-compatible format) "
                             "is always written; with --no-swa it is the candidate "
                             "artifact.")
    ...
    if args.rl_mode:
        ...
        # C5 window-membership check (2026-06-13, training-05/ops-quarantine-04): window
        # membership used to be filename-existence + human memory — a forgotten quarantine
        # silently rejoined invalid data for up to W-1 iterations (the Jun-8 crippled-window
        # incident class). Each window H5 must (a) NOT sit next to an INVALID marker file
        # (the quarantine mechanism: `touch INVALID` in the rl_iter_<k> dir instead of
        # moving files), and (b) carry the rl_parent_* lineage stamps vectorize_v2 writes
        # (an unstamped file is pre-stamp or foreign). Different PARENTS across the window
        # are LEGITIMATE (older entries were generated by ancestors under promote-unless-
        # harm) — the lineage is printed for the operator, not asserted equal.
        if not args.window_override:
            import h5py as _h5py
            for p in sp_paths:
                marker = os.path.join(os.path.dirname(os.path.abspath(p)), "INVALID")
                if os.path.exists(marker):
                    sys.exit(f"[RL] window file {p} is marked INVALID ({marker}) — an "
                             "invalidated generation run must not rejoin the training mix. "
                             "Remove it from the window or pass --window-override (forensic).")
                with _h5py.File(p, "r") as hf:
                    pb = hf.attrs.get("rl_parent_bin")
                    if pb is None:
                        sys.exit(f"[RL] window file {p} carries no rl_parent_bin lineage stamp "
                                 "— pre-stamp or foreign data. Re-vectorize with --stamp-parent "
                                 "or pass --window-override (forensic).")
                    print(f"[RL] window lineage: {os.path.basename(p)} <- generator {pb} "
                          f"(sha {str(hf.attrs.get('rl_parent_sha256'))[:12]}...)")
    ...
    # --- SWA setup (last 20% of epochs; --no-swa disables, J1 2026-06-13) ---
    swa_start_epoch = args.swa_start_epoch or max(1, int(args.epochs * 0.8))
    if args.no_swa:
        swa_start_epoch = args.epochs + 1   # never reached: no updates, no swa_model.pt
    ...
    # --- Save FINAL-epoch model (always; the candidate artifact under --no-swa) ---
    # Same checkpoint shape as swa_model.pt so export_weights_v2 / eval_deepsets_h5 /
    # the parity tools load it identically.
    final_ckpt = {
        "epoch": epoch,
        "global_step": global_step,
        "model_state_dict": model.state_dict(),
    ...
    if use_deepsets:
        final_ckpt["property_table_path"] = os.path.abspath(args.property_table)
    else:
        final_ckpt["state_dim"] = state_dim
        final_ckpt["hidden_dim"] = args.hidden_dim
        final_ckpt["num_layers"] = args.num_layers
        final_ckpt["value_only"] = args.value_only
    torch.save(final_ckpt, os.path.join(args.output_dir, "final_model.pt"))
    print(f"  Final-epoch model saved to {args.output_dir}/final_model.pt")
